...
+ [社区检测-图算法] | [Fast unfolding of communities in large networks](https://arxiv.org/pdf/0803.0476.pdf) | [阅读笔记](https://zhuanlan.zhihu.com/p/178790546) | 经典的Louvain算法，其是基于模块度优化的社区检测算法，且是一种启发式贪婪算法。流程是：（1）初始时将每个顶点当作一个社区，社区个数与顶点个数相同；（2）依次将每个顶点与之相邻顶点合并在一起，计算它们最大的模块度增益是否大于0，如果大于0，就将该结点放入模块度增量最大的相邻结点所在社区；（3）迭代第二步，直至算法稳定，即所有顶点所属社区不再变化；（4）将各个社区所有节点压缩成为一个结点，社区内点的权重转化为新结点环的权重，社区间权重转化为新结点边的权重；（5）重复步骤1-3，直至算法稳定。Louvain算法的优点是时间复杂度低（nlogn），适合大规模的网络、社区划分结果稳定且有具体指标、天然自带层次化。而缺点在于容易导致”过拟合“。 | Vincent D. Blondel et al, 2008
...
+ [社区检测-综述] | [Network Community Detection: A Review and Visual Survey](https://arxiv.org/ftp/arxiv/papers/1708/1708.00977.pdf) | [阅读笔记](https://zhuanlan.zhihu.com/p/141401358) | 一篇关于社区检测的综述，文章对社区检测概念和发展进行了介绍，并对当下社区检测算法按照分类进行了细致的介绍，包括传统的社区检测技术、基于分裂的社区检测技术、基于模块化优化的社区检测技术、重叠社区检测技术、动态社区检测技术等 | Bisma S. Khan et al, 2017
